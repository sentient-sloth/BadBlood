Function New-CustomADUser {
    ...
    if ($accountType -le 3){
        $nameSuffix = "SA"
        $SamAcctName = "$(Get-Random -Minimum 100 -Maximum 9999999999)" + "$nameSuffix"
    } else {
        $Surname = Get-Content (Join-Path $scriptpath 'Names\familynames-usa-top1000.txt') | Get-Random
        $Givenname = Get-Content (Join-Path $scriptpath 'Names\forenames-usa-top2000.txt') | Get-Random
        $SamAcctName = ((Get-Culture).TextInfo).ToTitleCase(("$Givenname.$Surname".ToLower()))
    }
    
    $passwordinDesc = 1..1000 | Get-Random
    
    $pass = New-SWRandomPassword -MinPasswordLength 22 -MaxPasswordLength 25
    if ($passwordinDesc -lt 10) { 
        $description = 'Just so I dont forget my password is: ' + $pass
    }
    if($SamAcctName.length -gt 20){
        $SamAcctName = $SamAcctName.substring(0,20)
    }
    
    try {
        $exists = Get-ADUser $SamAcctName -ErrorAction Stop
    } catch {
        $exists = $false
    }
    ...
    $UserParams = @{
        ...
        DisplayName       = ($SamAcctName -Replace '\.',' ')
        Name              = ($SamAcctName -Replace '\.',' ')
        ...
        UserPrincipalName = "$SamAcctName@$dnsroot"
    }
